...
export default defineComponent({
  ...
  props: {
    list: {
      //这里特别有一点，我们现在的 Array 是没有类型的，只是一个数组，我们希望它是一个 ColomnProps 的数组，那么我们是否可以使用了类型断言直接写成 ColomnProps[]，显然是不行的 ，因为 Array 是一个数组的构造函数不是类型，我们可以使用 PropType 这个方法，它接受一个泛型，將Array 构造函数返回传入的泛型类型。
  ...
  setup(props) {
    const columnList = computed(() => {
      ...
      return props.list.map((column) => {
        if (!column.avatar) {
          column.avatar = require("@/assets/ssb.jpg");
        }
        return column;
      });
    });
